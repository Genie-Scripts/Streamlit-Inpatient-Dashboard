...
from loader import read_excel_cached # process_uploaded_file は直接使用されていないため削除も検討
from forecast import generate_filtered_summaries
# utils.py から initialize_all_mappings をインポート
from utils import initialize_all_mappings, create_dept_mapping_table # create_dept_mapping_table は目標値ファイル処理の箇所で直接使用するため
    ...
    try:
        ...
        if validation_results.get("warnings", []):
            with st.expander("データ検証の警告", expanded=True):
                for warning_msg in validation_results["warnings"]: # warning -> warning_msg
        ...
        # === ここで initialize_all_mappings を呼び出す ===
        if df is not None and not df.empty:
            initialize_all_mappings(df, target_data) #
            print("診療科および病棟のマッピング情報を初期化・更新しました。")
        # ==========================================
...
def create_data_processing_tab(): #
    ...
    with col1:
        st.markdown("**メインデータファイル**") # キャッシュ利用時は任意
        base_file_uploader = st.file_uploader(
            "固定ファイル (Excel)",
            type=["xlsx", "xls"],
            key="base_file_dp_tab",
            help="病院の入院患者データを含むメインのExcelファイル。過去に処理したファイルと同じ場合は、キャッシュが利用されるためアップロード不要です。"
        )
    with col2:
        st.markdown("**補完データ (オプション)**")
    ...
    if new_files_uploader:
        ...
        for i, file_obj in enumerate(new_files_uploader): # file -> file_obj
            with st.expander(f"📋 {file_obj.name} の列名確認", expanded=False):
                show_excel_column_info(file_obj)
    ...
    if base_file_uploader:
        ...
        can_process = True
        st.info("過去に処理したベースデータ（Parquetキャッシュ）が存在します。固定ファイルをアップロードしない場合、このキャッシュを使用します。")
    elif new_files_uploader:
        can_process = True # 追加ファイルのみでも処理可能とする
                    ...
                    if validation_res.get("warnings") or validation_res.get("info") or validation_res.get("errors"):
                        with st.expander("データ検証結果", expanded=False): #
                            ...
                            for info_msg in validation_res.get("info", []):
                                st.info(info_msg)
                            for warning_msg_disp in validation_res.get("warnings", []): # warning_msg -> warning_msg_disp
                                st.warning(warning_msg_disp)
                if st.session_state.get('data_processed', False) and st.session_state.get('target_data') is not None: #
                    with st.expander("診療科マッピング設定", expanded=False): #
                        ...
                        try:
                            ...
                            if not dept_mapping or not st.session_state.get('dept_mapping_initialized', False): # 初期化フラグも確認
                                # create_dept_mapping_table(st.session_state.target_data) # ここでは呼び出さず、initialize_all_mappings に任せる
                                st.warning("診療科マッピングがまだ初期化されていません。データ処理後に自動的に行われます。")
                            dept_mapping = st.session_state.get('dept_mapping', {}) # 再取得
                            if dept_mapping:
...
def show_excel_column_info(uploaded_file): #
    ...
    try:
        ...
        try:
            ...
            with col1:
                st.write("**検出された列名:**")
                for i, col_name in enumerate(df_sample.columns): # col -> col_name
                    dtype_str = str(df_sample[col_name].dtype)
                    st.write(f"{i+1}. {col_name} ({dtype_str})")
            with col2:
                st.write("**期待される列名:**")
                expected = [
                    "病棟コード", "診療科名", "日付", "在院患者数",
                ...
                matched_columns = []
                for col_name_exp in expected: # col -> col_name_exp
                    if col_name_exp in df_sample.columns:
                        st.write(f"✅ {col_name_exp}")
                        matched_columns.append(col_name_exp)
                    else:
                        similar_cols = [c for c in df_sample.columns if col_name_exp.replace('患者数', '') in c or col_name_exp.replace('数', '') in c]
                        if similar_cols:
                            st.write(f"❓ {col_name_exp} (類似: {similar_cols[0]})")
                        else:
                            st.write(f"❌ {col_name_exp}")
            available_count = len(matched_columns)
            essential_columns = ["病棟コード", "診療科名", "日付"]
            essential_count = sum(1 for col_name_ess in essential_columns if col_name_ess in matched_columns) # col -> col_name_ess
            ...
            st.write("**データ品質チェック:**")
            quality_issues = []
            for col_name_match in matched_columns: # col -> col_name_match
                null_count = df_sample[col_name_match].isnull().sum()
                if null_count > 0:
                    quality_issues.append(f"'{col_name_match}': {null_count}個の欠損値")
            if quality_issues:
                st.warning("品質上の問題:")
