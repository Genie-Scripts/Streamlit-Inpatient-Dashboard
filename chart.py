...
def create_interactive_patient_chart(
    ...
    """
    患者数推移のPlotlyグラフ（目標ライン/達成ゾーン付き、Streamlit＆HTMLエクスポート両用）
    - data: DataFrame（'日付', '入院患者数（在院）' 必須）
    - days: 描画する直近日数（0の場合全期間）
    - target_line: 目標値（数値、任意）
    - target_zone: 達成ゾーン（[min, max] 形式、任意）
    """
...
def plotly_fig_to_base64_img(fig, width=900, height=350):
    """
    Plotly FigureをBase64 PNG画像として<img>タグで返す
    - fig: Plotly Figure
    - width/height: 画像サイズ
    """
...
# Streamlit画面用：st.plotly_chart(fig, use_container_width=True) でそのまま利用可能
# HTMLダッシュボード用：plotly_fig_to_base64_img(fig) でHTMLにimg埋め込み
