...
def create_patient_chart(data, title="入院患者数推移", days=90, show_moving_average=True, 
                        target_value=None, font_name_for_mpl=None):
    """データから患者数推移グラフを作成する（目標値対応強化版）- Matplotlib PDF用"""
    ...
    try:
        ...
        if target_value is not None:
            try:
                ...
                if target_val_float > 0:  # 0より大きい値のみ表示
                    ...
                    date_max = grouped["日付"].max()
                    
                    # 目標値ライン
                    ax.axhline(y=target_val_float, color='#9b59b6', linestyle='-.', 
                              linewidth=2, label=f'目標値: {target_val_float:.1f}', alpha=0.8)
                    
                    # 注意ゾーン（目標値の97%）
                    caution_threshold = target_val_float * 0.97
                    ax.fill_between([date_min, date_max], caution_threshold, target_val_float, 
                                   color='orange', alpha=0.15, 
                                   label='注意ゾーン(目標未達)')
                    
                    # 目標値を上回った場合の良好ゾーン
                    max_patient_count = grouped["入院患者数（在院）"].max()
                    if max_patient_count > target_val_float:
                        ax.fill_between([date_min, date_max], target_val_float, 
                                       max(max_patient_count, target_val_float * 1.1), 
        ...
        ax.legend(prop=legend_prop, loc='best')
        ...
        end_time = time.time()
        logger.info(f"グラフ '{title}' 生成完了 (目標値: {target_value}), 処理時間: {end_time - start_time:.2f}秒")
        return buf
...
def create_interactive_patient_chart(data, title="入院患者数推移", days=90, show_moving_average=True, 
    ...
    """
    インタラクティブな患者数推移グラフを作成する (Plotly) - 目標値対応強化版
    """
    try:
        if not isinstance(data, pd.DataFrame) or data.empty:
        ...
        if grouped.empty:
            logger.warning(f"create_interactive_patient_chart: '{title}' の期間絞り込み後データが空です。")
            return None
        ...
        fig.add_trace(go.Scatter(
            x=grouped["日付"], y=grouped["入院患者数（在院）"], 
        ...
        if not grouped.empty:
            date_min, date_max = grouped["日付"].min(), grouped["日付"].max()
            fig.add_trace(go.Scatter(
                x=[date_min, date_max], y=[avg, avg], 
                mode='lines', name=f'平均: {avg:.1f}', 
            ...
            if target_value is not None and pd.notna(target_value):
                try:
                    ...
                    if target_val_float > 0:
                        ...
                        # 目標値ライン
                        fig.add_trace(go.Scatter(
                            x=[date_min, date_max], y=[target_val_float, target_val_float], 
                            mode='lines', name=f'目標値: {target_val_float:.1f}', 
                        ...
                        caution_threshold = target_val_float * 0.97
                        fig.add_trace(go.Scatter(
                            x=[date_min, date_max, date_max, date_min], 
                            y=[caution_threshold, caution_threshold, target_val_float, target_val_float], 
                            fill='toself', fillcolor='rgba(255, 165, 0, 0.2)', 
                            line=dict(color='rgba(255,165,0,0)', width=0), 
                            name='注意ゾーン', hoverinfo='none'
                        ))
                        ...
                        if max_patient_count > target_val_float:
                            fig.add_trace(go.Scatter(
                                x=[date_min, date_max, date_max, date_min], 
                                y=[target_val_float, target_val_float, 
                                   max(max_patient_count, target_val_float * 1.1), 
                                   max(max_patient_count, target_val_float * 1.1)], 
                                fill='toself', fillcolor='rgba(144, 238, 144, 0.2)', 
...
def create_interactive_dual_axis_chart(data, title="入院患者数と患者移動の推移", days=90):
    ...
    try:
        ...
        if grouped.empty:
            logger.warning(f"create_interactive_dual_axis_chart: '{title}' の期間絞り込み後データが空です。")
            return None
        ...
        if "入院患者数（在院）_7日移動平均" in grouped.columns:
            fig.add_trace(go.Scatter(
                x=grouped["日付"], y=grouped["入院患者数（在院）_7日移動平均"], 
                name="入院患者数（在院）", line=dict(color="#3498db", width=3), mode="lines"
            ), secondary_y=False)
        ...
        for col, color_val in colors_map.items():
            ...
            if ma_col_name in grouped.columns:
                fig.add_trace(go.Scatter(
                    x=grouped["日付"], y=grouped[ma_col_name], 
                    name=col, line=dict(color=color_val, width=2), mode="lines"
                ), secondary_y=True)

        fig.update_layout(
            title=title, xaxis_title="日付", 
            legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="center", x=0.5), 
            hovermode="x unified", 
            font=dict(family="Arial, 'Noto Sans JP', sans-serif", size=12), 
            height=500
        )
...
@st.cache_data(ttl=1800, show_spinner=False)
def create_dual_axis_chart(data, title="入院患者数と患者移動の推移", filename=None, days=90, 
                          font_name_for_mpl=None):
    """
    ...
    try:
        ...
        end_time = time.time()
        logger.info(f"二軸グラフ '{title}' 生成完了, 処理時間: {end_time - start_time:.2f}秒")
        return buf
    except Exception as e:
