...
# 既存のCSSブロックの直後に以下を追加してください

# 🚀 Streamlit Cloud 強力対応版（JavaScript + CSS）
st.markdown("""
<style>
/* 最高優先度CSS - Streamlit Cloud専用 */
html body div.stApp * [data-testid="stMetricValue"] {
    font-size: 14px !important;
    word-break: break-all !important;
    overflow-wrap: break-word !important;
    line-height: 1.0 !important;
...
html body div.stApp * [data-testid="stMetricLabel"] {
...
.stApp [data-testid="metric-container"] [data-testid="stMetricLabel"] {
    font-size: 9px !important;
    white-space: nowrap !important;
    overflow: hidden !important;
    text-overflow: ellipsis !important;
...
function forceFixMetrics() {
    console.log('🔥 Force fixing all metrics...');
    
    // すべてのメトリック値を強制修正
    const metricValues = document.querySelectorAll('[data-testid="stMetricValue"]');
    console.log(`Found ${metricValues.length} metric values to fix`);
    
    metricValues.forEach((element, index) => {
        // 現在のフォントサイズをログ出力
        const currentSize = window.getComputedStyle(element).fontSize;
        console.log(`Metric ${index}: Current size = ${currentSize}`);
        ...
        // 修正後のサイズをログ出力
        const newSize = window.getComputedStyle(element).fontSize;
        console.log(`Metric ${index}: New size = ${newSize}`);
    ...
    console.log(`Found ${metricLabels.length} metric labels to fix`);
    ...
    columns.forEach(col => {
        ...
        col.style.setProperty('padding', '0 3px', 'important');
    });
...
// 複数のタイミングで実行
setTimeout(forceFixMetrics, 100);
setTimeout(forceFixMetrics, 500);
setTimeout(forceFixMetrics, 1000);
setTimeout(forceFixMetrics, 2000);
setTimeout(forceFixMetrics, 3000);

// 定期的に実行（3秒ごと）
setInterval(forceFixMetrics, 3000);
...
const observer = new MutationObserver((mutations) => {
    ...
    mutations.forEach(mutation => {
        if (mutation.type === 'childList') {
            mutation.addedNodes.forEach(node => {
                if (node.nodeType === 1 && (
                    (node.querySelector && node.querySelector('[data-testid="stMetric"]')) ||
                    (node.matches && node.matches('[data-testid="stMetric"]'))
                )) {
                    needsUpdate = true;
                }
            });
    ...
    if (needsUpdate) {
        console.log('🔄 DOM updated, reapplying fixes...');
        setTimeout(forceFixMetrics, 100);
    }
});
...
console.log('🚀 Powerful Streamlit Cloud Fixer Loaded!');
</script>
""", unsafe_allow_html=True)
