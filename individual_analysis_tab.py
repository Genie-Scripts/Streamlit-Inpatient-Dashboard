...
# --- lambda内でdelするためのヘルパー関数 ---
def _clear_session_state_key(key_to_delete):
...
def display_individual_analysis_tab(df_filtered_main): # 引数としてフィルタリング済みDFを受け取る
    ...
    if not current_results_data or not isinstance(current_results_data, dict) or current_results_data.get("summary") is None:
        ...
        with pdf_col1:
            ...
            session_pdf_data_key = f'pdf_data_portrait_{portrait_dl_button_key}'
            session_filename_key = f'pdf_filename_portrait_{portrait_dl_button_key}'
            ...
            if st.button("📄 縦向きPDF出力", key=portrait_button_key, use_container_width=True):
                if chart_data_for_graphs is None or chart_data_for_graphs.empty:
                    ...
                    with st.spinner(f'{current_filter_title_display}の縦向きPDFを生成中...'):
                        try:
                            pdf_data_portrait_bytes = create_pdf( # 戻り値は BytesIO オブジェクトを期待
                            ...
                            if pdf_data_portrait_bytes:
                                st.session_state[session_pdf_data_key] = pdf_data_portrait_bytes.getvalue() # getvalue()でバイト列を取得
                                st.session_state[session_filename_key] = f"入院患者数予測_{''.join(c if c.isalnum() or c in ['-', '_'] else '_' for c in current_filter_title_display)}_{latest_data_date.strftime('%Y%m%d')}.pdf"
                            else: st.error("縦向きPDFの生成に失敗しました。")
                        except Exception as e:
                            logger.error(f"縦向きPDF生成エラー: {e}", exc_info=True)
                            st.error(f"縦向きPDFの生成中にエラーが発生しました: {e}")
            
            if session_pdf_data_key in st.session_state:
                st.download_button(
                    label="📥 縦向きPDFをダウンロード",
                    data=st.session_state[session_pdf_data_key],
                    file_name=st.session_state[session_filename_key],
                    mime="application/pdf",
                    key=portrait_dl_button_key, 
                    on_click=_clear_session_state_key, # ヘルパー関数を使用
                    args=(session_pdf_data_key,) # 削除するキーを渡す
                )
                # filenameキーもクリアする場合は、ヘルパーを拡張するか、別のon_clickを設定
                if session_filename_key in st.session_state:
                    _clear_session_state_key(session_filename_key)
        ...
        with pdf_col2:
            ...
            session_pdf_data_key_land = f'pdf_data_landscape_{landscape_dl_button_key}'
            session_filename_key_land = f'pdf_filename_landscape_{landscape_dl_button_key}'
            ...
            if st.button("📄 横向きPDF出力", key=landscape_button_key, use_container_width=True):
                if chart_data_for_graphs is None or chart_data_for_graphs.empty:
                    ...
                    with st.spinner(f'{current_filter_title_display}の横向きPDFを生成中...'):
                        try:
                            pdf_data_landscape_bytes = create_landscape_pdf(
                                forecast_df=pdf_forecast_df_data,
                                df_weekday=current_results_data.get("weekday"),
                                df_holiday=current_results_data.get("holiday"),
                                df_all_avg=current_results_data.get("summary"),
                            ...
                            if pdf_data_landscape_bytes:
                                st.session_state[session_pdf_data_key_land] = pdf_data_landscape_bytes.getvalue()
                                st.session_state[session_filename_key_land] = f"入院患者数予測_{''.join(c if c.isalnum() or c in ['-', '_'] else '_' for c in current_filter_title_display)}_{latest_data_date.strftime('%Y%m%d')}_横向き.pdf"
                            else: st.error("横向きPDFの生成に失敗しました。")
                        except Exception as e:
                            logger.error(f"横向きPDF生成エラー: {e}", exc_info=True)
                            st.error(f"横向きPDFの生成中にエラーが発生しました: {e}")

            if session_pdf_data_key_land in st.session_state:
                st.download_button(
                    label="📥 横向きPDFをダウンロード",
                    data=st.session_state[session_pdf_data_key_land],
                    file_name=st.session_state[session_filename_key_land],
                    mime="application/pdf",
                    key=landscape_dl_button_key,
                    on_click=_clear_session_state_key,
                    args=(session_pdf_data_key_land,)
                )
                if session_filename_key_land in st.session_state:
                     _clear_session_state_key(session_filename_key_land)
