                    ...
                    with st.spinner(f'{current_filter_title_display}の縦向きPDFを生成中...'):
                        try:
                            pdf_data_portrait = create_pdf(
                                ...
                                df_weekday=current_results_data.get("weekday"), df_holiday=current_results_data.get("holiday"),
                                df_all_avg=current_results_data.get("summary"), chart_data=chart_data_for_graphs,
                                title_prefix=current_filter_title_display, latest_date=latest_data_date,
                                target_data=target_data, filter_code=filter_code_for_target, graph_days=[pdf_graph_days_to_use]
                            )
                            if pdf_data_portrait:
                                ...
                                st.download_button(
                                    label="📥 縦向きPDFをダウンロード", data=pdf_data_portrait, file_name=filename_pdf,
                                    mime="application/pdf", key=portrait_dl_button_key
                                )
                    ...
                    with st.spinner(f'{current_filter_title_display}の横向きPDFを生成中...'):
                        try:
                            ...
                            if pdf_data_landscape:
                                ...
                                st.download_button(
                                    label="📥 横向きPDFをダウンロード", data=pdf_data_landscape, file_name=filename_pdf_land,
                                    mime="application/pdf", key=landscape_dl_button_key
                                )
