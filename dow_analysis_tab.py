...
import numpy as np
import datetime
from datetime import timedelta, date # date をインポート
...
def display_dow_analysis_tab(df, start_date, end_date, common_config=None):
    ...
    # 関数冒頭で start_date と end_date を Timestamp に統一
    # app.py からは Timestamp で渡される想定だが、念のため変換
    try:
    ...
    st.markdown(f"<div style='font-size: 14px; color: #666; margin-bottom:1rem;'>選択期間: {start_date_ts.strftime('%Y年%m月%d日')} ～ {end_date_ts.strftime('%Y年%m月%d日')}</div>", unsafe_allow_html=True)
    ...
    if calculate_dow_summary:
        summary_df = calculate_dow_summary(
            df=df, # df全体を渡す。calculate_dow_summary内部で日付フィルタリングする想定
            start_date=start_date_ts, # Timestamp を渡す
            end_date=end_date_ts,     # Timestamp を渡す
        ...
        if summary_df is not None and not summary_df.empty:
            ...
            if cols_to_show_summary and len(cols_to_show_summary) > 3:
                ...
                st.download_button(
                    ...
                    data=csv_summary,
                    file_name=f"曜日別サマリー_{selected_unit_dow}_{start_date_ts.strftime('%Y%m%d')}-{end_date_ts.strftime('%Y%m%d')}.csv",
                    mime='text/csv',
                )
            else:
    ...
    try:
        data_min_ts = df['日付'].min() # pd.Timestamp
        data_max_ts = df['日付'].max() # pd.Timestamp
        
        # UI用の日付範囲 (datetime.date型)
        data_min_date_ui = data_min_ts.date()
        data_max_date_ui = data_max_ts.date()
    except Exception as e:
        st.error(f"データの日付範囲取得でエラーが発生しました: {e}")
        return
    
    # セッション状態の日付がデータ範囲外ならクリア (datetime.date で比較)
    for key in ['dow_comparison_start_date', 'dow_comparison_end_date']:
        if key in st.session_state:
            stored_date_val = st.session_state[key]
            if isinstance(stored_date_val, date) and (stored_date_val < data_min_date_ui or stored_date_val > data_max_date_ui):
    ...
    if enable_comparison:
        ...
        with col1_comp:
            # セッションから比較開始日を取得 (datetime.date型)
            session_comp_start_dt = st.session_state.get("dow_comparison_start_date")
            
            # デフォルト比較開始日を計算 (start_date_ts は Timestamp)
            ideal_comp_start_ts = start_date_ts - pd.Timedelta(days=365)
            
            default_comp_start_dt_val: date # 型ヒント
            if session_comp_start_dt and isinstance(session_comp_start_dt, date) and \
               data_min_date_ui <= session_comp_start_dt <= data_max_date_ui:
                default_comp_start_dt_val = session_comp_start_dt
            # ideal_comp_start_ts.date() で datetime.date に変換して比較・代入
            elif ideal_comp_start_ts.date() >= data_min_date_ui:
                default_comp_start_dt_val = ideal_comp_start_ts.date()
            else:
                # 1年前がデータ範囲外なら、データ開始日から90日後 (ただし最大日を超えない)
                calculated_default = data_min_date_ui + timedelta(days=90)
                default_comp_start_dt_val = min(calculated_default, data_max_date_ui)
            
            # 最終的な範囲チェック (datetime.date型で)
            default_comp_start_dt_val = max(default_comp_start_dt_val, data_min_date_ui)
            default_comp_start_dt_val = min(default_comp_start_dt_val, data_max_date_ui)

            comp_start_date_dt = st.date_input( # 返り値は datetime.date
                "比較期間：開始日", 
                value=default_comp_start_dt_val,
        ...
        with col2_comp:
            session_comp_end_dt = st.session_state.get("dow_comparison_end_date")
            
            # 現在の分析期間の長さを計算 (datetime.date型同士で)
            period_length_days = (end_date_ts.date() - start_date_ts.date()).days
            # デフォルト比較終了日を計算 (comp_start_date_dt は datetime.date)
            ideal_comp_end_dt = comp_start_date_dt + timedelta(days=period_length_days)
            
            default_comp_end_dt_val: date # 型ヒント
            if session_comp_end_dt and isinstance(session_comp_end_dt, date) and \
               data_min_date_ui <= session_comp_end_dt <= data_max_date_ui and \
               session_comp_end_dt >= comp_start_date_dt: # 開始日以降であることも確認
                default_comp_end_dt_val = session_comp_end_dt
            elif ideal_comp_end_dt <= data_max_date_ui and ideal_comp_end_dt >= comp_start_date_dt:
                default_comp_end_dt_val = ideal_comp_end_dt
            else:
                default_comp_end_dt_val = data_max_date_ui # データ最大日をデフォルトに
            
            # 最終的な範囲チェック (datetime.date型で)
            default_comp_end_dt_val = max(default_comp_end_dt_val, comp_start_date_dt) # 開始日以降に
            default_comp_end_dt_val = min(default_comp_end_dt_val, data_max_date_ui)   # データ最大日以前に
            default_comp_end_dt_val = max(default_comp_end_dt_val, data_min_date_ui)   # データ最小日以降に (念のため)


            comp_end_date_dt = st.date_input( # 返り値は datetime.date
                "比較期間：終了日", 
                value=default_comp_end_dt_val,
                min_value=comp_start_date_dt, # 動的に開始日を最小値に設定
                max_value=data_max_date_ui,
                key="dow_comparison_end_date"
            )
        
        if st.button("現在期間と同じ長さに設定", key="set_same_length"):
            period_len_days = (end_date_ts.date() - start_date_ts.date()).days
            # st.session_state.dow_comparison_start_date は st.date_input の結果なので datetime.date
            current_comp_start_dt_from_session = st.session_state.dow_comparison_start_date 
            
            target_comp_end_dt = current_comp_start_dt_from_session + timedelta(days=period_len_days)
            
            # データの範囲内に収める
            if target_comp_end_dt > data_max_date_ui:
                target_comp_end_dt = data_max_date_ui
                # 開始日も調整 (データ最小日より前にならないように)
                current_comp_start_dt_from_session = max(data_min_date_ui, target_comp_end_dt - timedelta(days=period_len_days))
            
            st.session_state.dow_comparison_start_date = current_comp_start_dt_from_session
            st.session_state.dow_comparison_end_date = target_comp_end_dt
            st.experimental_rerun()
        
        if comp_start_date_dt > comp_end_date_dt: # datetime.date同士の比較はOK
            st.error("比較期間の終了日は開始日以降に設定してください。")
            return # ここでreturnしないと以降の処理でエラーの可能性
        
        comp_dow_data = None # 初期化
        if selected_chart_metrics:
            comp_dow_data = get_dow_data(
                df=df, # df全体を渡す。get_dow_data内部で日付フィルタリングする想定
                unit_type=selected_unit_dow,
                target_items=target_items_dow,
                start_date=pd.Timestamp(comp_start_date_dt), # Timestampに変換して渡す
                end_date=pd.Timestamp(comp_end_date_dt),     # Timestampに変換して渡す
                metric_type=metric_type_for_logic,
                patient_cols_to_analyze=selected_chart_metrics
            )
            ...
            if comp_dow_data is not None and not comp_dow_data.empty:
                if comparison_display_mode == "縦に並べて表示":
                    current_chart_fig = create_dow_chart(
                        dow_data_melted=dow_data_for_chart, # これは現在の分析期間のデータ
                        unit_type=selected_unit_dow,
                        target_items=target_items_dow,
                        metric_type=metric_type_for_logic,
                        patient_cols_to_analyze=selected_chart_metrics,
                    ...
                    comp_chart_fig = create_dow_chart(
                        dow_data_melted=comp_dow_data, # 比較期間のデータ
                        unit_type=selected_unit_dow,
                        target_items=target_items_dow,
                        metric_type=metric_type_for_logic,
                        patient_cols_to_analyze=selected_chart_metrics,
                    ...
                    if dow_data_for_chart is not None and comp_dow_data is not None:
                        current_period_name = f"現在期間 ({start_date_ts.strftime('%Y/%m/%d')}～{end_date_ts.strftime('%Y/%m/%d')})"
                        comp_period_name = f"比較期間 ({comp_start_date_dt.strftime('%Y/%m/%d')}～{comp_end_date_dt.strftime('%Y/%m/%d')})" # datetime.dateで表示
                        
                        # dow_data_for_chart と comp_dow_data が None でないことを確認
                        if dow_data_for_chart is not None:
                            dow_data_for_chart['期間'] = current_period_name
                        if comp_dow_data is not None:
                             comp_dow_data['期間'] = comp_period_name
                        
                        # 両方のデータフレームが存在する場合のみ結合
                        if dow_data_for_chart is not None and comp_dow_data is not None:
                            combined_data = pd.concat([dow_data_for_chart, comp_dow_data], ignore_index=True)
                        elif dow_data_for_chart is not None:
                            combined_data = dow_data_for_chart
                            st.warning("比較期間のデータがありません。現在期間のみ表示します。")
                        elif comp_dow_data is not None:
                            combined_data = comp_dow_data
                            st.warning("現在期間のデータがありません。比較期間のみ表示します。")
                        else:
                            combined_data = pd.DataFrame() # 空のデータフレーム
                        ...
                        comp_summary_df = None # 初期化
                        if calculate_dow_summary:
                            # safe_date_filter に渡す日付も Timestamp
                            comparison_filtered_df = safe_date_filter(df, pd.Timestamp(comp_start_date_dt), pd.Timestamp(comp_end_date_dt))
                            
                            if comparison_filtered_df is not None and not comparison_filtered_df.empty:
                                comp_summary_df = calculate_dow_summary(
                                    df=comparison_filtered_df, 
                                    start_date=pd.Timestamp(comp_start_date_dt), # Timestamp
                                    end_date=pd.Timestamp(comp_end_date_dt),     # Timestamp
                                    group_by_column=group_by_col_name,
                                    target_items=target_items_dow
                                )
                            else:
...
def display_dow_analysis_tab(df, start_date, end_date, common_config=None):
    ...
    for key in ['dow_comparison_start_date', 'dow_comparison_end_date']:
        if key in st.session_state:
            stored_date_val = st.session_state[key]
            if isinstance(stored_date_val, date) and \
               (stored_date_val < data_min_date_ui or stored_date_val > data_max_date_ui):
                del st.session_state[key]
    ...
    if enable_comparison:
        ...
        with col1_comp:
            session_comp_start_dt = st.session_state.get("dow_comparison_start_date")
            ideal_comp_start_ts = start_date_ts - pd.Timedelta(days=365) # start_date_ts は Timestamp
            
            default_comp_start_dt_val: date
            if session_comp_start_dt and isinstance(session_comp_start_dt, date) and \
               data_min_date_ui <= session_comp_start_dt <= data_max_date_ui:
                default_comp_start_dt_val = session_comp_start_dt
            elif ideal_comp_start_ts.date() >= data_min_date_ui: # .date() で比較
                default_comp_start_dt_val = ideal_comp_start_ts.date() # .date() で代入
            else:
                calculated_default = data_min_date_ui + timedelta(days=90)
                default_comp_start_dt_val = min(calculated_default, data_max_date_ui)
            ...
            comp_start_date_dt = st.date_input( # 返り値は datetime.date
                "比較期間：開始日", 
                value=default_comp_start_dt_val, # datetime.date を渡す
                min_value=data_min_date_ui,
                max_value=data_max_date_ui,
                key="dow_comparison_start_date"
            )
        ...
        with col2_comp:
            session_comp_end_dt = st.session_state.get("dow_comparison_end_date")
            period_length_days = (end_date_ts.date() - start_date_ts.date()).days # date型で計算
            ideal_comp_end_dt = comp_start_date_dt + timedelta(days=period_length_days) # comp_start_date_dt は date
            
            default_comp_end_dt_val: date
            if session_comp_end_dt and isinstance(session_comp_end_dt, date) and \
               data_min_date_ui <= session_comp_end_dt <= data_max_date_ui and \
               session_comp_end_dt >= comp_start_date_dt:
                default_comp_end_dt_val = session_comp_end_dt
            elif ideal_comp_end_dt <= data_max_date_ui and ideal_comp_end_dt >= comp_start_date_dt:
                default_comp_end_dt_val = ideal_comp_end_dt
            else:
                default_comp_end_dt_val = data_max_date_ui
            
            default_comp_end_dt_val = max(default_comp_end_dt_val, comp_start_date_dt)
            default_comp_end_dt_val = min(default_comp_end_dt_val, data_max_date_ui)
            default_comp_end_dt_val = max(default_comp_end_dt_val, data_min_date_ui)

            comp_end_date_dt = st.date_input( # 返り値は datetime.date
                "比較期間：終了日", 
                value=default_comp_end_dt_val, # datetime.date を渡す
                min_value=comp_start_date_dt, 
                max_value=data_max_date_ui,
                key="dow_comparison_end_date"
            )
        ...
        if st.button("現在期間と同じ長さに設定", key="set_same_length"):
            period_len_days = (end_date_ts.date() - start_date_ts.date()).days
            current_comp_start_dt_from_session = st.session_state.dow_comparison_start_date # date型
        ...
        if comp_start_date_dt > comp_end_date_dt:
            st.error("比較期間の終了日は開始日以降に設定してください。")
            return 
        ...
        if selected_chart_metrics:
            comp_dow_data = get_dow_data(
                ...
                unit_type=selected_unit_dow,
                target_items=target_items_dow,
                start_date=pd.Timestamp(comp_start_date_dt), # Timestampに変換
                end_date=pd.Timestamp(comp_end_date_dt),     # Timestampに変換
                metric_type=metric_type_for_logic,
                patient_cols_to_analyze=selected_chart_metrics
            )
            ...
            if comp_dow_data is not None and not comp_dow_data.empty:
                if comparison_display_mode == "縦に並べて表示":
                    ...
                    # ... (既存の「1つのグラフで比較」ロジック、グラフ生成部分は変更なし) ...
                    # ただし、comp_summary_df の生成部分で日付の型変換を確認
                    if dow_data_for_chart is not None and comp_dow_data is not None:
                        current_period_name = f"現在期間 ({start_date_ts.strftime('%Y/%m/%d')}～{end_date_ts.strftime('%Y/%m/%d')})"
                        comp_period_name = f"比較期間 ({comp_start_date_dt.strftime('%Y/%m/%d')}～{comp_end_date_dt.strftime('%Y/%m/%d')})"
                        ...
                        # 期間比較のインサイト
                        st.markdown("<div class='info-card'>", unsafe_allow_html=True) # グラフの後に
                        st.markdown("#### <span style='color: #191970;'>期間比較インサイト</span>", unsafe_allow_html=True)
